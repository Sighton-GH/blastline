...
await q.evaluate(() => { __blastlineTest.setPoints(9e9); });
let found = false;
for (let i = 0; i < 14 && !found; i++) {
  found = await q.evaluate(() => {
    const T = __blastlineTest;
    if (T.getState().state !== 'armory') {
      T.pause();
      [...document.querySelectorAll('button')].find(b => /^SHOP$/i.test(b.textContent.trim()))?.click();
    }
    const cards = [...document.querySelectorAll('.armory-card, .shop-card, [data-upgrade]')];
    const names = cards.map(c => c.textContent.slice(0, 40));
    return names.some(n => /taming/i.test(n));
  });
  if (!found) await q.evaluate(() => {
    document.querySelector('.armory-continue')?.click();
    [...document.querySelectorAll('button')].find(b => /^RESUME$/i.test(b.textContent.trim()))?.click();
  });
  await q.waitForTimeout(150);
}
console.log('taming card visible:', found);
await q.screenshot({ path: '/home/sandbox/c6-evidence/174-taming-card.png' });
