...
await q.evaluate(() => {
  ...
  const buy = ['damage','fireRate','damage','fireRate','reinforcements','reinforcements','damage','multishot','damage','reinforcements'];
  let n = 0;
  for (const id of buy) { T.purchase(id); n += 1; if (n % 2 === 0) cont(); }
  cont();
  T.setWave(3); T.setWaveTime(0); T.freeze(false);
});
// steered run until boss engages
let shot = false;
for (let i = 0; i < 400 && !shot; i++) {
  const s = await q.evaluate(() => {
    ...
    T.advance(.5);
    return { state: st.state, boss: st.bossArchetype, bt: st.bossTime, troops: st.troops };
  });
  if (s.state === 'game-over') { console.log('died before boss'); break; }
  if (s.boss && s.bt > 2.5) {
    await q.evaluate(() => __blastlineTest.freeze(true));
    await q.waitForTimeout(150);
    console.log('boss engaged:', JSON.stringify(s));
    await q.screenshot({ path: '/home/sandbox/c6-evidence/177-boss-fight.png' });
    shot = true;
  }
