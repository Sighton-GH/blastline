...
while (Date.now() - t0 < 200000) {
  await q.evaluate(() => {
    const T = __blastlineTest;
    const st = T.getState();
    if (st.state === 'boss') T.setPlayerX(0.43); // park at the right lane edge once the boss fight starts - the stall condition
    else {
  ...
  const s = await q.evaluate(() => { const st = __blastlineTest.getState(); return { state: st.state, wt: +(st.waveTime||0).toFixed(0), bt: +(st.bossTime||0).toFixed(0), boss: st.bossArchetype, gate: st.bossDmgGate, hp: st.bossHp, bx: st.bossX===null?null:+st.bossX.toFixed(2), troops: st.troops, en: st.activeEnemies, score: st.score }; });
