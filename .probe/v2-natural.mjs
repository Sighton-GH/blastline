...
const DIFF = process.env.DIFF || 'recruit';
await page.goto(`http://127.0.0.1:${port}/?qa&diff=${DIFF}`, { waitUntil: 'load' });
await page.waitForTimeout(500);
await page.evaluate(() => { const want = (new URLSearchParams(location.search)).get('diff') || 'recruit'; const b = document.querySelectorAll('#difficultyPicker button'); const idx = { recruit: 0, veteran: 1, elite: 2 }[want]; if (b[idx]) b[idx].click(); }); await page.waitForTimeout(200); await page.tap('#playBtn');
