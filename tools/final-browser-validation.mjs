...
async function runInteractionValidation() {
  ...
  record('bossRewardPickUnlocksArmoryShop', shopCount === 16 && continueEnabled, { shopCount, continueEnabled });
  ...
  record('sixteenArmoryChoices', rewards.length === 16 && new Set(rewards).size === 16, rewards);
  await page.locator('.reward-card:not(:disabled)').first().click();
  await page.locator('.armory-continue').click();
