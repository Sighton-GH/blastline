...
async function runInteractionValidation() {
  ...
  record('bossRewardPickUnlocksArmoryShop', shopCount === 10 && continueEnabled, { shopCount, continueEnabled });
  ...
  record('tenArmoryChoices', rewards.length === 10 && new Set(rewards).size === 10, rewards);
  await page.locator('.reward-card:not(:disabled)').first().click();
  await page.locator('.armory-continue').click();
